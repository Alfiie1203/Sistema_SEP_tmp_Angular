...
@Component({
  selector: 'app-listar-x-correo',
  templateUrl: './listar-x-correo.component.html',
  styleUrls: ['./listar-x-correo.component.css']
})
export class ListarXCorreoComponent implements OnInit {
  ...
  getInformacionCorreo(){
    ...
    //alexagalloz9@gmail.com | pedro.gonfrier@prestigecol.com | gerencia@frupalma.com | dgonzalez@numar.net
        if (this.informacion[i]["correoElectronico"] == this.email) {
          this.informacionCorreo.push(this.informacion[i])   
        }
      }
    });
  }
